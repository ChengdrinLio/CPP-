...
enum ENUM_MSG_TYPE
{
    ...
    ENUM_MSG_TYPE_USR_Offline_RESPOND, //用户下线请求
...
struct PDU
{
    ...
    uint uiMsgType;    //消息类型
    char caData[64];   //字符数据[64字节]
    uint uiMsgLen;     //实际消息长度（变化的）
    int caMsg[];       //实际消息（空数组=指针）
};

PDU * mkPDU(uint uiMsgLen); //传个变化的
